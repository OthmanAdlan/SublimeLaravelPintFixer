...
def load_settings():
    return sublime.load_settings("SublimeLaravelPintFixer.sublime-settings")
...
def pint_possible_paths():
    paths = []

    if is_windows():
        executable_name = "pint.bat"
    else:
...
class Logger:
    ...
    def console(self, msg):
        print("Larave Pint Fixer: {0}".format(msg))

    def debug(self, msg):
        if self.settings.get("debug"):
            print("Larave Pint Fixer (DEBUG): {0}".format(msg))
...
class FixerProcess:
    ...
    def get_configured_laravel_pint_path(self):
        path = self.settings.get_expanded('path')

        if not path:
            path = self.locate_laravel_pint()

        if not path:
            raise ExecutableNotFoundException("Couldn't find laravel pint")
    ...
    def locate_laravel_pint(self):
        paths = pint_possible_paths()
        for path in paths:
            self.logger.debug("looking for laravel pint at: " + path)
            if is_executable_file(path):
                self.logger.console("autodetected: " + path)
                return path

        self.logger.console("laravel pint file not found")
...
class ViewFormatter:
    ...
    def format_contents(self, contents):
        """
        Write the contents in a temporary file, format it with laravel pint and returns the formatted contents.
...
class SublimeLaravelPintFixListener(sublime_plugin.EventListener):
    def on_pre_save(self, view):
        if setting_enabled('on_save'):
            view.run_command('sublime_laravel_pint_fix')
